...
def detect_heads(xml_name):
	# uploaded_xml_name = '.'.join(xml_name.split(".")[0:-1]) + ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(8)) + ".xml"
	detected_img_name = "conf_"+xml_name+".jpg"

	cascPath = app.root_path + "/static/"+xml_name
	print(cascPath)
	print(os.path.join(app.config['UPLOAD_FOLDER'], "conf.jpg"))
	faceCascade = cv2.CascadeClassifier(cascPath)

	image = cv2.imread(os.path.join(app.config['UPLOAD_FOLDER'], "conf.jpg"))
